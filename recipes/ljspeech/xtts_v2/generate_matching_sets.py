...
class KNN_VC_Wrapper:
    ...
    def match_list(
        ...
        query_seq = query_seq.cpu().detach()
...
profile_dir = Path('/home/hltcoe/xli/ARTS/TTS/recipes/ljspeech/xtts_v2/exp/profiles')
...
with open(speaker_file, 'r') as f:
    for line in f:
        ...
        if len(parts) > 2 and parts[1] != 'SEX' and 'dev' in parts[2]:
            spk_id, spk_set = parts[0].strip(), parts[2].strip()
            speakers[spk_id] = spk_set
...
# Generate num_profiles
for speaker_id in tqdm(speaker_ids):
    # Retrieve Reference WAV Paths
    speaker_dir = data_dir / speakers[speaker_id]/ speaker_id
    ref_wav_paths = [str(f) for f in speaker_dir.rglob('*.flac')]
    
    # Generate Matching Sets
    with torch.no_grad():
        matching_sets_and_index = knn_vc.get_matching_set(ref_wav_paths)
    matching_sets = matching_sets_and_index[0]

    # Save the profile
    torch.save(
        matching_sets,
        str(profile_dir / Path(speaker_id).with_suffix('.pt'))
    )
    faiss.write_index(
        matching_sets_and_index[1], 
        os.path.join(profile_dir, speaker_id + ".index")
    )
