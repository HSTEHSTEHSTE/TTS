...
profile_path = '/home/hltcoe/xli/ARTS/TTS/recipes/ljspeech/xtts_v2/exp/profiles/100'
# source_path = '/home/hltcoe/xli/ARTS/TTS/corpora/commonvoice/en/clips'
source_path = '/home/hltcoe/xli/ARTS/anon_baseline/data/LibriSpeech/dev-other'
# extension = '.mp3'
extension = '.flac'
target_path = '/exp/xli/ARTS/librispeech_norm'
...
recon_index = faiss.read_index(profile_path + '.index')
index = [faiss.index_cpu_to_gpu(res, 0, recon_index)]
recon_index = [recon_index]
matching_set = [torch.load(profile_path + '.pt')]

wavs = list(Path(source_path).rglob('*' + extension))
with torch.no_grad():
    for wav in tqdm(wavs):
        wav_name = str(wav).split('/')[-1][:-len(extension)]

        # Feature Extraction
        query_seq = knn_vc.get_features(str(wav))
        ...
        out_wav = knn_vc.vocode(out_feats.unsqueeze(0)).cpu()

        # Save Output
        output_path = os.path.join(target_path, wav_name + '.wav')
